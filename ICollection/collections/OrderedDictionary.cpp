...
void OrderedDictionary::add(IKey *k, ICollectible *val)
{
    ...
    {
        if(key->equals(current->getKey())){  // la clave ya está, se reemplaza
            current->setVal(val);
    ...
    ++size;
    OrderedDictionaryEntry *newEntry = new OrderedDictionaryEntry(dynamic_cast<OrderedKey*>(key->clone()), val);
    if(parent == NULL)
        root = newEntry;
...
void OrderedDictionary::remove(IKey *k)
{
    ...
    while(current != NULL)
{
    if(key->equals(current->getKey()))  // la clave ya está, se puede borrar
        break;
    parent = current;
    parentComparison = key->compare(current->getKey());
    if(parentComparison == LESSER)
